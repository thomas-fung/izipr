...
M_bioChem1 <- glm.izip(art ~ ., data = bioChemists,
                       ref.lambda = 1)
...
test_that("Testing function that extracting info from glm.izip", {
  ...
  expect_equal(residuals(M_bioChem),
  ...
  expect_equal(predict(M_bioChem, type = "link"),
                  predict(M_bioChem))
  expect_snapshot(predict(M_bioChem, se.fit = TRUE))
  expect_equal(predict(M_bioChem, type = "link", se.fit = TRUE),
...
test_that("Testing broom for iZIP", {
  ...
  expect_equal(augment(M_bioChem),
  ...
  expect_equal(augment(M_bioChem, newdata = newdataframe,
                       type.predict = "link"),
               augment(M_bioChem, newdata = newdataframe))
  expect_snapshot(augment(M_bioChem, newdata = newdataframe, se_fit = TRUE))
  expect_equal(augment(M_bioChem, newdata = newdataframe, se_fit = TRUE),
               augment(M_bioChem, newdata = newdataframe, type.predict = "link",
                     se_fit = TRUE))
  expect_snapshot(augment(M_bioChem,
                        newdata = newdataframe,
                        type.predict = "response"))
  expect_snapshot(augment(M_bioChem, newdata = newdataframe,
